...
class IQN(nn.Module):
    def __init__(self, state_size, action_size, layer_size, n_step, seed, N, dueling=False, noisy=False, device="cuda:0"):
        ...
        self.input_shape = [self.input_shape[2], self.input_shape[1], self.input_shape[0]]
        ...
        if self.state_dim == 3:
            self.head = nn.Sequential(
                nn.Conv2d(3, out_channels=32, kernel_size=8, stride=4),
                nn.ReLU(),
    ...
    def forward(self, input, num_tau=8):
        ...
        """
        _input = [input[0], input[3], input[1], input[2]]
        batch_size = _input.shape[0]
        
        x = torch.relu(self.head(_input))
        if self.state_dim == 3: x = x.view(_input.size(0), -1)
